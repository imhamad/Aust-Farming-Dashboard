<?php 
	...
	$custom_userpage 	= Kirki::get_option( 'workscout', 'pp_login_workscout_page' );
...
<ul class="float-right">
	...
	if ( is_user_logged_in() ) { 
		if( ! empty( $custom_userpage )) { 
			...
			if($user_page_status){	?>
			<li>
				<a href="<?php echo esc_url(apply_filters('workscout_userpage', $userpage_link)); ?>"><i class="fa fa-sign-out"></i> <?php esc_html_e('User Page','workscout') ?></a>
			</li>
		<?php 
			}
		} ?>
...
<?php } else { //user not logged in
	...
	if(!$login_popup) {
		...
		?>
			<li><a href="<?php echo esc_url($register_page_url); ?>#tab-register"><i class="fa fa-user"></i> <?php esc_html_e('Sign Up','workscout') ?></a></li>
