...
add_action( 'admin_notices', 'workscout_update_api_notice' );
function workscout_update_api_notice() {
        $api_key = Kirki::get_option( 'workscout','pp_maps_browser_api', '');
        if( empty($api_key)) {
            ?>
            <div class="error notice">
                <p><?php _e( 'Hi! Since last changes in Google Maps <strong>it\'s required now to provide API key</strong> to be able to include maps on your page. You can do it in <a href="customize.php">Customize - Map Options</a>. Visit this <a target="_blank" href="https://console.developers.google.com/flows/enableapi?apiid=maps_backend,geocoding_backend,directions_backend,distance_matrix_backend,elevation_backend&keyType=CLIENT_SIDE&reusekey=true">link</a> to generate API key for your website. More detailed documentation is <a href="http://kb.purethemes.net/article/73-google-maps-api-key-for-workscout">here</a>', 'workscout' ); ?></p>
            </div>
            <?php
        }
    }
