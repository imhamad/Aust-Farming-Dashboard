	...
	public function output_colors() {
		...
		foreach ( $terms as $term ) {
			...
			if($color){
				...
				echo ".job-spotlight span.{$term->slug}, .map-box span.job-type.{$term->slug},.new-layout.job_listings > li a span.job-type.{$term->slug} { color: #{$color}; border: 1px solid #{$color}; background-color: rgba( {$hexcolor},0.07) }";
